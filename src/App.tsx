...
function App() {
  ...
  return (
    <div className="bg-brand-dark min-h-screen font-sans text-brand-text flex flex-col selection:bg-brand-gold selection:text-black">
      ...
      <div className="bg-brand-gold text-black py-2 px-4 hidden md:flex justify-between items-center text-sm font-bold tracking-wide">
        <div className="flex gap-6 max-w-7xl mx-auto w-full px-4">
          <span className="flex items-center gap-2">
            ...
            servicio@refaccionestoluca.com
          </span>
      ...
      {/* Navbar Responsiva */}
      <nav className="sticky top-0 w-full z-50 bg-brand-black/95 backdrop-blur-lg border-b border-gray-800 shadow-xl">
        ...
        <div className={`lg:hidden absolute top-full left-0 w-full bg-brand-black/95 backdrop-blur-xl border-b border-gray-800 transition-all duration-300 ${isMobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible h-0 overflow-hidden'}`}>
          <div className="flex flex-col px-4 py-6 gap-6 text-center font-bold text-gray-300 uppercase tracking-widest">
            <button onClick={() => handleNav('home')} className="hover:text-brand-gold">Inicio</button>
            <button onClick={() => handleNav('about')} className="hover:text-brand-gold">Nosotros</button>
            <button onClick={() => handleNav('services')} className="hover:text-brand-gold">Servicios</button>
            <button onClick={() => handleNav('catalog')} className="hover:text-brand-gold">Catálogo</button>
            <button onClick={() => handleNav('reviews')} className="hover:text-brand-gold">Reseñas</button>
            <button onClick={() => handleNav('contact')} className="bg-brand-gold text-black px-6 py-4 rounded-sm font-black hover:bg-yellow-500 w-full flex justify-center items-center gap-2">
              UBICACIÓN →
            </button>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8 pb-12 grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div>
            ...
            <ul className="space-y-3 text-gray-400 font-medium">
              <li onClick={() => handleNav('home')} className="hover:text-brand-gold cursor-pointer transition-colors flex items-center gap-2"><span className="text-brand-gold text-lg leading-none">⌂</span> Inicio</li>
              <li onClick={() => handleNav('about')} className="hover:text-brand-gold cursor-pointer transition-colors flex items-center gap-2"><span className="text-brand-gold text-lg leading-none">ⓘ</span> Sobre Nosotros</li>
              <li onClick={() => handleNav('services')} className="hover:text-brand-gold cursor-pointer transition-colors flex items-center gap-2"><span className="text-brand-gold text-lg leading-none">⚙</span> Servicios</li>
              <li onClick={() => handleNav('catalog')} className="hover:text-brand-gold cursor-pointer transition-colors flex items-center gap-2"><span className="text-brand-gold text-lg leading-none">☰</span> Catálogo de Piezas</li>
              <li onClick={() => handleNav('reviews')} className="hover:text-brand-gold cursor-pointer transition-colors flex items-center gap-2"><span className="text-brand-gold text-lg leading-none">☆</span> Reseñas</li>
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-4 text-gray-400 font-medium">
              ...
              <li className="flex items-center gap-2"><svg className="w-5 h-5 text-brand-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg> Estado de México Toluca</li>
            </ul>
          </div>
        </div>
